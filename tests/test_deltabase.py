...
def test_register_with_alias(db):
    db.upsert(table="test_table", primary_key="id", data=LazyFrame([dict(id=1, name="alice", job="teacher")]))
    db.upsert(table="test_table", primary_key="id", data=LazyFrame([dict(id=2, name="john", job="chef")]))
    db.commit("test_table")

    db.register(table="test_table", alias="other_table")

    result = db.sql("select * from other_table", dtype="polars")

    assert isinstance(result, DataFrame)
    assert result.shape == (2, 3)
    assert set(result["id"].to_list()) == set([1,2])
    assert set(result["name"].to_list()) == set(["alice","john"])
